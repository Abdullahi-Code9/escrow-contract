...
fn test_upgrade_try_update_closure_correct_for_all_inputs() {
    ...
    let result: Result<u32, Error> = None::<u32>.unwrap_or(1).checked_add(1).ok_or(Error::InvalidAmount);
    assert_eq!(result, Ok(2));

    // Present key: increments by exactly one.
    let result: Result<u32, Error> = Some(1u32).unwrap_or(1).checked_add(1).ok_or(Error::InvalidAmount);
    assert_eq!(result, Ok(2));

    let result: Result<u32, Error> = Some(41u32).unwrap_or(1).checked_add(1).ok_or(Error::InvalidAmount);
    assert_eq!(result, Ok(42));

    // One below maximum: succeeds.
    let result: Result<u32, Error> = Some(u32::MAX - 1).unwrap_or(1).checked_add(1).ok_or(Error::InvalidAmount);
    assert_eq!(result, Ok(u32::MAX));

    // Maximum: overflow returns InvalidAmount, not a panic or wrap.
    let result: Result<u32, Error> = Some(u32::MAX).unwrap_or(1).checked_add(1).ok_or(Error::InvalidAmount);
